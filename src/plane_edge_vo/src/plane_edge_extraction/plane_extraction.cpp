...
//		std::cout<<theta<<", "<<phy<<", "<<d<<std::endl;
	...
	void PlaneExtraction::extractPlanes(Scan *scan)
	{
		fp.open("plane_extraction.txt",std::ios::app);
		if(debug)
		{
...
//							tmp_plane->num_points=tmp_plane->points.size();
						...
						if(debug)
						{
							...
							fp<<"\tnormal="<<tmp_plane->normal.transpose()<<std::endl;
							fp<<"\td="<<tmp_plane->d<<std::endl;
							fp<<"\tavg_rgb="<<tmp_plane->avg_rgb.transpose()<<std::endl;
							fp<<"\tplane size - "<<tmp_plane->points.size()<<std::endl;
						}
						...
						if(scan->observed_planes.size()>0)
						{
							for(int i_plane=0;i_plane<scan->observed_planes.size();i_plane++)
							{
								...
								double tmp_cos_angle=tmp_plane->normal.transpose()*scan->observed_planes[i_plane]->normal;
								if(tmp_cos_angle>0.9999)
								{
									tmp_cos_angle=1.0;
								}
								Eigen::Vector3d tmp_delta_rgb=tmp_plane->avg_rgb-scan->observed_planes[i_plane]->avg_rgb;
								if(debug)
								{
...
//									*allplane = *plane + *allplane;
									if(debug)
									{
										...
										fp<<"\t\tnormal="<<scan->observed_planes[i_plane]->normal.transpose()<<std::endl;
										fp<<"\t\td="<<scan->observed_planes[i_plane]->d<<std::endl;
										fp<<"\t\tavg_rgb="<<scan->observed_planes[i_plane]->avg_rgb.transpose()<<std::endl;
										fp<<"\t\tsize="<<scan->observed_planes[i_plane]->points.size()<<std::endl;
									}
									break;
								}
							}
						...
						if(have_same_plane == false)
						{
							...
							tmp_plane->index=scan->observed_planes.size();
...
//							*allplane = *plane + *allplane;
		...
		if(debug)
		{
			...
			{
				fp<<"\tnormal="<<scan->observed_planes[i]->normal.transpose();
	...
	void PlaneExtraction::computePlaneAvgCov(Plane *plane)
	{
		Eigen::Vector3d tmp;
		if(plane->points.size()>0)
		{
			plane->avg_xyz.setZero();
			plane->avg_rgb.setZero();
			plane->cov_xyz.setZero();
			plane->cov_rgb.setZero();
			for(std::vector<Point>::iterator it=plane->points.begin();it!=plane->points.end();++it)
			{
				plane->avg_xyz=plane->avg_xyz+it->xyz;
				plane->avg_rgb=plane->avg_rgb+it->rgb;
			}
			plane->avg_xyz=plane->avg_xyz/plane->points.size();
			plane->avg_rgb=plane->avg_rgb/plane->points.size();
			for(std::vector<Point>::iterator it=plane->points.begin();it!=plane->points.end();++it)
			{
				tmp=it->xyz-plane->avg_xyz;
				plane->cov_xyz=plane->cov_xyz+tmp*tmp.transpose();
				tmp=it->rgb-plane->avg_rgb;
				plane->cov_rgb=plane->cov_rgb+tmp*tmp.transpose();
			}
			plane->cov_xyz=plane->cov_xyz/plane->points.size();
			plane->cov_rgb=plane->cov_rgb/plane->points.size();
		}
	}

	void PlaneExtraction::fusePlanes(Plane *tmp_plane, Plane *fuse)
	{
		for(std::vector<Point>::iterator it=tmp_plane->points.begin();it!=tmp_plane->points.end();++it)
		{
			fuse->points.push_back(*it);
		}
	}
