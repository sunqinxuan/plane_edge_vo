...
namespace ulysses
{
	bool LoopClosing::findLoops(Map *map, Scan *scan)
	{
		...
		ANNpointArray map_poses=annAllocPts(map->seq.size(),3);
		Transform Tgc;
		for(size_t i=0;i<map->seq.size();i++)
		{
			Tgc=map->seq[i]->Tcg.inv();
			map_poses[i][0]=Tgc.t(0);
			map_poses[i][1]=Tgc.t(1);
			map_poses[i][2]=Tgc.t(2);
		}
		kdtree=new ANNkd_tree(map_poses,map->seq.size(),3);
		Tgc=scan->Tcg.inv();
		cur_pose[0]=Tgc.t(0);
		cur_pose[1]=Tgc.t(1);
		cur_pose[2]=Tgc.t(2);
...
//			std::cout<<"findLoops "<<std::endl;
			for(size_t i=0;i<candidates;i++)
			{
				if(index[i]==ANN_NULL_IDX)
					continue;
				LoopClosure loop(scan,map->seq[index[i]]);
