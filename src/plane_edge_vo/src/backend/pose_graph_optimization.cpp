...
namespace ulysses
{
	double PoseGraphOptimization::optimize(Map *map)
	{
		...
		for(size_t i=0;i<map->seq.size();i++)
		{
			...
			g2o::SE3Quat Tcg(map->seq[i]->Tcg.R,map->seq[i]->Tcg.t);
//			g2o::SE3Quat Tcg(map->seq[i]->Tcg_gt.R,map->seq[i]->Tcg_gt.t);
//			pose->setEstimate(Tcg.inverse());
			...
			if(i>0)
			{
				int id_cur=map->seq[i]->id;
				int id_ref=map->seq[i]->scan_ref->id;
				...
				edge->setVertex(0,v0);
				edge->setVertex(1,v1);
				g2o::SE3Quat Tcr(map->seq[i]->Tcr.R,map->seq[i]->Tcr.t);
...
//			g2o::SE3Quat Tcg=v->estimate().inverse();
			...
			map->seq[i]->Tcg.R=Tcg.rotation().toRotationMatrix();
			map->seq[i]->Tcg.t=Tcg.translation();
		}
